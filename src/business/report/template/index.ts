...
import { BaseOptionDataMap, EChartsDataDisplayerControl } from "../displayers/echarts/index";
...
export function getEChartsDataDisplayerCreateFunction<T extends BaseOptionDataMap>(option: EChartsOptions, dataMapClass: new () => T): (
    ...
    return (dataMapAttribute, id, customOption) => {
        ...
        let displayer = createObject(
            EChartsDataDisplayerControl,
            {
                id: id || newGuid(),
                option: merginOption,
                dataMap: createObject(dataMapClass, dataMapAttribute),
                theme: 'gov-net-thing'
            }
        );
        return displayer;
